...
app-new-symfony-stable-docker:
	$(DOCKER) exec -it $(PHP) bash -c 'symfony new application-$(APP_NAME) --no-git --docker --debug --version="stable" --php="8.2" --dir="/var/www/html/$(APP_NAME)" '

app-new-symfony-stable-raw:
	symfony new application-stable-$(APP_NAME) \
		--no-git \
		--docker \
		--debug \
    	--version='stable' \
    	--php='8.2' \
    	--dir='/var/www/html/application-stable-$(APP_NAME)' \
